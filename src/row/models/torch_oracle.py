...
class LearnedOperator(nn.Module):
    ...
    def __init__(self, d: int, rank: int, alpha: float, seed: int) -> None:
        super().__init__()
...
class OracleCompositor(nn.Module):
    ...
    def __init__(self, d: int, rank: int, operators: int, alpha: float, seed: int) -> None:
        super().__init__()
        self.operators = nn.ModuleList(
            LearnedOperator(d=d, rank=rank, alpha=alpha, seed=seed + 997 * index)
