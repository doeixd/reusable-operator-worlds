...
# Route-margin normalization: SURVIVES as computed, but it re-detects "stream 0 fails" (Tier 0, 2026-09-16)

Plan `ROUTE_MARGIN_NORMALIZATION_PLAN.md`, module and tests committed `0d01702`
before any number existed. Tier 0, 24 frozen stage-2 libraries, no training, no
new world. All guards passed, including the recovery gate (the generating route
was the argmin in 16/16 draws in EVERY library, so Test B's measure works).
`check_prereg.py` and `check_invalid.py` pass. Report:
`reports/route_margin_normalization.json`.

**Registered triage: SURVIVES**, on Test A alone.

| test | result |
|---|---|
| A, `route_margin` concordance | **k = 5/7** (expected 2.33, exact Binomial(7,1/3) tail **0.0453**) |
| A, `self_margin` concordance | k = 3/7 (tail 0.429) - chance |
| A, stage-2 ERROR reference | k = 1/7 (tail 0.941) - worse than chance |
| B, `self_margin` pooled | rho **+0.143**, permutation 0.264, separation 1.11 |
| within-world z-scored `route_margin` | rho -0.221 (pooled was -0.514) |
| `self_margin` vs `route_margin` | rho +0.365 |

**Why the label should not be read as a mechanism:**
1. **Test A largely re-detects which stream failed.** In 6 of 7 worlds the
   worst-outcome stream is stream 0 or 1, and the 5 concordant worlds are exactly
   those where the lowest margin sits on that same stream. Stream 0 is the
   canonical replay seed the world-quality census already identified as the usual
   failing stream, so this may be the same fact in another coordinate.
2. **The purpose-built comparable measure found nothing.** `self_margin` is
   library-intrinsic (targets generated by the library's own routes, no teacher
   task set) and gives +0.143 with permutation 0.264 - and the SIGN is opposite to
   `route_margin`'s pooled -0.514. Its recovery gate is clean, so this is a real
   null, not a broken instrument.
3. **The scale artifact is visible.** Within-world z-scoring drops the
   `route_margin` correlation from -0.514 to -0.221, which is what one expects if
   the pooled figure was mostly cross-world scale.
4. **Two cells break any monotone story.** SO4 w6 s1 has the HIGHEST `self_margin`
   (0.441) and fails at 2.17; SO4 w9 s2 has nearly the lowest (0.225) and passes at
   0.016.
5. **The label hangs on one marginal binary test:** 5 of 7 coin-flip decisions at
   p = 0.045. One world flipping gives k = 4 and MIXED.

**A tension to record, not gloss.** The stage-2 ERROR reference scores 1/7 on the
same within-world test, i.e. prefix error is worse than chance at picking which
stream will fail. The world-quality census reported a 2.4-3.1x fail/pass
separation on prefix error *within mixed-outcome worlds*; that used medians over
cells pooled across those worlds, whereas this test asks per world which single
stream is worst. Both are correct as computed and they answer different questions,
but the pooled separation should not be read as "prefix error identifies the
failing stream".

**Conclusion, across all three censuses.** Prefix error, slot geometry and route
identifiability have each been measured on the cells in hand. None of them
survives as a mechanism: error separates pooled but not per world, slot
duplication carries nothing, and route identifiability survives only in the
