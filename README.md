...
| **N1** | Is it the curriculum's ORDER or the mere PRESENCE of short tasks? | `ANCHORS_SUFFICE` 3/3 (offline, worlds 0-2). 124 anchors pooled with the length-3 tasks in one training set (the offline trainer has no task order), length never revealed: 0.0093 / 0.0101 / 0.0066. A pool-matched sham with identical minibatch draws is worse than no anchors at all (~1.05). Presence, not order. |
| **N1b / N1c** | Which anchors, and how many? | `L1_SUFFICES_ONLY`, `k* = 32`: length-1 anchors alone reach 0.0067-0.0175, and 64 length-2 anchors do nothing (1.12-1.16). Then `K6_INSUFFICIENT`, `COUNT_COMPENSATES`: 6 length-1 anchors fail even with full operation coverage, while 18 covering 5 of 6 operations pass in 2 of 3 worlds, where the library forms the missing operation from compositions. A quota of single-operation tasks, not a curriculum. |
| **O1** | Do order-free anchors work ONLINE? (Tier 1, worlds 10-12) | `LIVE`, exploratory. One random-order online lifetime passes 2 of 3 worlds (0.037 / 0.473 / 0.018), as often as the staged curriculum (0.272 / 0.027 / 0.019), but in a different pair of worlds. Order isn't needed, and it doesn't buy reliability. O2 (worlds 13-19, three streams, frozen `b3c1c85`) asks the reliability question. |
| **O2** | Is order-free online formation RELIABLE? (Tier 2, worlds 13-19 x 3 streams) | `ORDER_FREE_UNRELIABLE`. Passing cells of 21: order-free 9, curriculum 12, no length-2 tasks 4; no anchors 0/7. None is reliable (>= 19/21). The curriculum reproduces its historical ~0.58 rate. The order-free stream rarely collapses but often misses narrowly. L1-L8 close, per a rule registered before the run. |
| **O2C** | Is the online deficit convergence or structure? (Tier 1, O2's saved models) | **Convergence.** 8,192 more updates on seen data: 20/21 pass (from 9), with nothing broken. A deployable replay-only 'sleep' (4 examples per task) reaches 15/21, also with nothing broken. The one collapsed cell stays collapsed. What the learner retains is the next lever. |
| **O2D** | How much must the learner RETAIN for sleep to work? (Tier 1) | A reservoir of 64 examples per task matches the all-data ceiling (20/21); 4-16 per task reach 18/21; nothing is broken at any level. At small memory, near-threshold cells depend on which examples are kept. Collapses are never rescued. |
| **O2E** | Does sleep help the other protocols? Can it undo a collapse? (Tier 1) | It repairs every near-miss in the curriculum (12 -> 18/21) and the no-length-2 stream (4 -> 13/21), and breaks nothing. It rescues 1 of 10 collapses. Online formation has two failure modes: under-convergence, which sleep fixes, and collapse, which it doesn't. |
| **O2G** | Is the curriculum's collapse determined, or chance? (Tier 1) | **Both.** Re-running stage 3 from a collapsed run's stage-2 library brings back the early onset disruption 6 of 6 times, but it collapses only 3 of 6. Healthy libraries collapse 1 of 36. The risk is library-linked, the outcome ~50% chance, and there's an early warning, so rollback-and-retry is a candidate mechanism. |
